...
class RoundtableSession(Base, TimestampMixin):
    ...
    Contains multiple rounds of agent interactions for coding tasks.
    """
    ...
    # Relationships
    project: Mapped[Optional["Project"]] = relationship("Project", backref="roundtable_sessions")
...
class RoundtableRound(Base, TimestampMixin):
    ...
    Each round contains a task prompt and multiple agents that respond.
    """
...
class RoundtableAgent(Base, TimestampMixin):
    ...
    Represents an AI model (Anthropic/OpenAI) with a specific role.
    """
...
class RoundtableResponse(Base, TimestampMixin):
    ...
    Stores the content, vote (if applicable), and usage metrics.
    """
